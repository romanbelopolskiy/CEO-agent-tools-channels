...
Binding constraints: feature spec before behavior changes; ADR + C4/architecture-doc updates for durable architecture decisions; layered architecture; new durable agent-system state targets InsForge Postgres unless an ADR records an exception; Claude agents do not connect to Telegram directly; no secrets in docs/logs/commits; cost-impacting infra requires Roman approval; Done includes docs update, validation/check, and secret scan.

...
├── package.json            # v3.1.6, deps: @modelcontextprotocol/sdk
...
| Command | Trigger patterns | tmux keys sent |
|---------|-----------------|----------------|
| `stop` | `stop`, `/stop`, `стоп`, `esc`, `escape` | `Escape` |
| `status` | `status`, `/status`, `статус` | `Escape`, then (150ms delay), `/status Enter` |
| `compact` | `compact`, `/compact`, `компакт` | `Escape`, then (150ms delay), `/compact Enter` |
...
5. For each batch in `cmd.tmuxKeys`: if not the first batch, wait 150ms (lets the CLI repaint after Escape), then call `execFileSync("tmux", ["send-keys", "-t", botName, ...keys])`.
6. `statusManager.finishTask(task.taskId)` — marks the interrupted task as done so the next task gets a fresh status message.
7. `stopTyping(botName, chatId)` — clears the Telegram "typing…" indicator immediately.
